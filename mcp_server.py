...
try:
    ...
    from edit_existing_impl import (
        list_existing_drafts, load_existing_draft, save_loaded_draft, is_loaded_draft,
    )
...
# Default output folder for saved drafts. Saving here makes drafts appear
# directly inside the CapCut desktop app. Override with the CAPCUT_DRAFT_FOLDER
# environment variable. Falls back to the standard Windows CapCut location.
...
TOOLS = [
    ...
    {
        "name": "save_draft",
        "description": "保存草稿 (saves into the CapCut Projects folder by default so it opens in the app)",
        "inputSchema": {
            "type": "object",
            "properties": {
                "draft_id": {"type": "string", "description": "草稿ID"},
                "draft_folder": {"type": "string", "description": "Output folder. Defaults to the CapCut Projects directory so the draft appears in the app."}
            }
        }
    },
    {
        "name": "list_drafts",
        "description": "List existing CapCut projects in the Projects folder so one can be picked to edit. Returns each project's folder name (use as draft_id for load_draft), display name, and whether it is editable.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "draft_folder": {"type": "string", "description": "Override the CapCut Projects folder. Defaults to the standard location."}
            }
        }
    },
    {
        "name": "load_draft",
        "description": "Open an EXISTING CapCut project for editing. Makes a safe COPY (the original is never modified) and loads it so add_video/add_text/etc. can be applied, then save_draft saves the copy in place. Returns a NEW draft_id to use for all subsequent edits.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "draft_id": {"type": "string", "description": "Folder name of the existing project (from list_drafts)."},
                "draft_folder": {"type": "string", "description": "Override the CapCut Projects folder. Defaults to the standard location."},
                "new_name": {"type": "string", "description": "Name for the edited copy. Defaults to '<original> (AI edit)'."}
            },
            "required": ["draft_id"]
        }
    }
        ...
        with capture_stdout() as captured:
            if tool_name == "create_draft":
                ...
                result = {
                    ...
                    "draft_url": f"http://localhost:9001/draft/downloader?draft_id={draft_id}"
                }
                ...
                result = list_existing_drafts(arguments.get("draft_folder"))

            elif tool_name == "load_draft":
                result = load_existing_draft(
                    arguments["draft_id"],
                    draft_folder=arguments.get("draft_folder"),
                    new_name=arguments.get("new_name"),
                )

            elif tool_name == "save_draft":
                # If this is a draft we loaded from an existing project, save the
                # edited COPY in place (preserving all original files). Otherwise
                # save a new draft into CapCut's Projects dir so it shows in the app.
                if is_loaded_draft(arguments.get("draft_id")):
                    result = save_loaded_draft(arguments["draft_id"])
                else:
                    if "draft_folder" not in arguments or not arguments.get("draft_folder"):
                        arguments["draft_folder"] = CAPCUT_DRAFT_FOLDER
                    save_result = save_draft_impl(**arguments)
                    if isinstance(save_result, dict) and "draft_url" in save_result:
                        result = {"draft_url": save_result["draft_url"], "saved_to": os.path.join(arguments["draft_folder"], arguments.get("draft_id", ""))}
                    else:
                        result = {"draft_url": f"http://localhost:9001/draft/downloader?draft_id=unknown"}
...
def handle_request(request_data: str) -> Optional[str]:
    """处理JSON-RPC请求"""
    try:
        # Strip a leading UTF-8 BOM / whitespace some clients prepend before parsing
        request = json.loads(request_data.strip().lstrip("﻿"))
