...
def download_script(draft_id: str, draft_folder: str = None, script_data: Dict = None) -> Dict[str, str]:
    ...
    try:
        ...
        # SECURITY PATCH: the remote third-party endpoint
        # (cut-jianying-vdvswivepm.cn-hongkong.fcapp.run) has been disabled to
        # keep this tool fully local. Remote draft retrieval is not used by the
        # normal save flow.
        if script_data is None:
            raise RuntimeError(
                "Remote draft retrieval is disabled in this local install. "
                "download_script requires script_data to be supplied explicitly."
            )
        else:
...
if __name__ == "__main__":
    # SECURITY PATCH: removed leftover test call that contacted a remote
    # endpoint with the original author's hardcoded local path.
    pass
